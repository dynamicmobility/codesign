...
from codesign.hyperdesigners.variants.mo_design_mlp import augment_observation_size as mo_augment_observation_size
...
def load_mo_design_mlp(
    ...
    """Load a design-conditioned MLP and its saved parameters from a checkpoint."""
    if network_factory is None:
        _, network_factory = setup_mo_design_mlp(config)
    params_config, params = _load_checkpoint(config, path, quiet)
    ...
    def augmented_network_factory(observation_size, action_size, **kwargs):
        return network_factory(
            observation_size=mo_augment_observation_size(
                observation_size, design_dim, num_objectives
            ),
            action_size=action_size,
            design_dim=design_dim,
            num_objectives=num_objectives,
