...
DATASET = 'results/Sep10/test/sweep_2d.npz'
...
def main():
    grid = codesign.Grid.load(DATASET)
    print(grid.rewards.shape)
    print(grid.designs.shape)
    print(grid.tradeoffs.shape)
    # quit()
    num_obj = grid.rewards.shape[-1]
    norm = Normalize(vmin=grid.designs.min(), vmax=grid.designs.max())
    ...
    for objs in combinations(range(num_obj), 2):
        ...
        rewards  = grid.mean_rewards[:, on_face].reshape(-1, num_obj)
        designs  = grid.designs[:, on_face].reshape(-1, grid.design_dim)
        # quit()
        ...
        mop.plot_pareto(
            ax        = ax,
            pareto    = rewards[:, objs],
            colors    = codesign.get_colors(norm(designs[:,[0]]), cmap=CMAP)[:, 0, :],
            objective = [TRADEOFF_LABELS[o] for o in objs],
            dominated_alpha = 0.2,
            dominated_s = 8,
            nondominated_s  = 30,
